...
![CI](https://github.com/ejunior029/fatal-accident-prediction-br/actions/workflows/ci.yml/badge.svg)
...
├── tests/                  automated tests + a small synthetic data fixture (data/models aren't real)
├── scripts/                one-off utilities (e.g. a placeholder model for CI's Docker build check)
├── models/                 trained models + tuned threshold (.joblib / .json, gitignored)
├── reports/                generated charts (.png, gitignored)
├── .github/workflows/      CI: lint, tests, Docker build — runs on every push
...
└── requirements-dev.txt    requirements-api.txt + pytest/httpx for running the test suite
```
...
**Run the test suite** — the tests don't need the real dataset or the trained model (neither is versioned); they run against a tiny synthetic fixture in `tests/fixtures/`:
...
- [x] Add automated tests for `src/data.py` and `src/api.py` + CI (lint, tests, Docker build) on every push
